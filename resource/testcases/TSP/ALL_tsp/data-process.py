    ...
    try:
        with open(filename, 'r') as file:
            ...
            for i, line in enumerate(lines):
                if "NODE_COORD_SECTION" in line or (
                        len(line.strip().split()) == 3 and line.strip().split()[0].isdigit()):
...
def read_path_sequence(filename):
    ...
    try:
        with open(filename, 'r') as file:
            ...
            start_idx = 0
            for i, line in enumerate(lines):
                if "TOUR_SECTION" in line or (
                        line.strip().isdigit() and not line.startswith(("NAME", "TYPE", "DIMENSION"))):
